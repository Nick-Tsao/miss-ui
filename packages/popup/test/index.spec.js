...
test('popup lock scroll', () => {
  ...
  expect(document.body.classList.contains('ms-overflow-hidden')).toBeTruthy();
  ...
  wrapper2.vm.$destroy();
  expect(document.body.classList.contains('ms-overflow-hidden')).toBeFalsy();
});
...
test('get container with parent', () => {
  ...
  const popup = wrapper.find('.ms-popup').element;
...
test('render overlay', () => {
  ...
  expect(div.querySelector('.ms-overlay')).toBeTruthy();
});
...
test('close on click modal', () => {
  ...
  const modal = div.querySelector('.ms-overlay');
